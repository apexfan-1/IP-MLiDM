    ...
    if(finderror($mass, $begin,$end)) {
        ...
        while ($mini != -999999) { 
            ...
            for ($i = 0; $i < count($mass); $i++) {
                ...
                if (!array_search($i, $index) && $mass[$mini][$i] != '0') { 
                    $temp = $point[$mini] + $mass[$mini][$i];
                    if ($point[$i] > $temp) {
                        $point[$i] = $temp;
        ...
        $_SESSION['mas'] ='Массив:<br><br>' .$insert.'<br>Начальная точка: '.$begin.'<br>Конечная точка:'.$end;
